...
import numpy as np
from q_controller import set_angles, servo_on, servo_off
from configs import my_quad
...
def main():
    ...
    while running:
        ...
        if pygame.K_1 in key_down:
            angles[0][0] += angle_delta
            angles[1][0] += angle_delta
            angles[2][0] += angle_delta
            angles[3][0] += angle_delta
        if pygame.K_2 in key_down:
            angles[0][0] -= angle_delta
            angles[1][0] -= angle_delta
            angles[2][0] -= angle_delta
            angles[3][0] -= angle_delta
        if pygame.K_3 in key_down:
            angles[0][1] += angle_delta
            angles[1][1] += angle_delta
            angles[2][1] += angle_delta
            angles[3][1] += angle_delta
        if pygame.K_4 in key_down:
            angles[0][1] -= angle_delta
            angles[1][1] -= angle_delta
            angles[2][1] -= angle_delta
            angles[3][1] -= angle_delta
        if pygame.K_5 in key_down:
            angles[0][2] += angle_delta
            angles[1][2] += angle_delta
            angles[2][2] += angle_delta
            angles[3][2] += angle_delta
        if pygame.K_6 in key_down:
            angles[0][2] -= angle_delta
            angles[1][2] -= angle_delta
            angles[2][2] -= angle_delta
            angles[3][2] -= angle_delta
